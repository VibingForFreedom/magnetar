...
func (s *server) read(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			s.logger.Debug("could not unmarshal packet data", "error", err)
			continue
		}
...
func (s *server) handleQuery(ctx context.Context, msg dht.RecvMsg) {
	...
	if retErr != nil {
		...
		if ok := errors.As(retErr, dhtErr); ok {
			...
			s.logger.Error("server error", "msg", msg, "retErr", retErr)
		}
	} else {
		res.R = &ret
	}
	...
	if sendErr := s.send(msg.From, res); sendErr != nil {
		s.logger.Debug("could not send response", "msg", msg, "retErr", sendErr)
	}
}
...
func (s *server) Query(
	...
	if sendErr := s.send(addr, msg); sendErr != nil {
		s.logger.Debug("could not send query", "msg", msg, "sendErr", sendErr)
		err = sendErr
